...
int* twoSum(int* nums, int numsSize, int target, int* returnSize) {
    int* result = (int*)malloc(2 * sizeof(int));       // Allocate memory for the result (2 indices)
    for (int i=0; i<numsSize; i++) {                   // Iterate through each element of the array
        for (int j= i+1; j<numsSize; j++) {            // For each element, check if there is another element that adds up to the target -> O(n^2) time complexity -> must be done better
            if (nums[i] + nums[j] == target) {         // If the sum of nums[i] and nums[j] equals the target, store the indices
                result[0]=i;
                result[1]=j;
